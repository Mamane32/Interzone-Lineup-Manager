...
export default async function CoachAppLayout({
  ...
  return (
    <div className="min-h-screen bg-coach-bg pb-24">
      <header className="sticky top-0 z-20 border-b border-black/5 bg-ink text-white">
        <div className="mx-auto flex max-w-md items-center justify-between px-4 py-3">
          <div className="flex items-center gap-2">
            {team.logo_url ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={team.logo_url} alt="" className="h-9 w-9 rounded-full object-cover ring-2 ring-amber-signal/30" />
            ) : (
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-xs font-display">
          ...
          <form action={coachLogout.bind(null, params.token)}>
            <button
              type="submit"
              className="flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs text-white/50 hover:bg-white/5 hover:text-white"
            >
