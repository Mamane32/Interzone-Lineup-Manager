...
export function generateStaticParams() {
  return STUDIO_SECTIONS.map((s) => ({ section: s.key }));
}

export default function StudioSectionPage({ params }: { params: { section: string } }) {
  const section = STUDIO_SECTIONS.find((s) => s.key === params.section);
  if (!section) notFound();

  return (
    <main className="mx-auto max-w-3xl px-5 py-10 sm:px-8">
      <Link href="/studio" className="mb-6 flex items-center gap-1.5 text-xs font-semibold text-white/50 hover:text-white">
        <ChevronLeft size={14} /> Studio
      </Link>
      <WorldHero world={world} dense />
      <div className="mt-6">
        <ReservedSectionNav world={world} sections={STUDIO_SECTIONS} activeKey={section.key} />
      </div>
      <div className="mt-8">
